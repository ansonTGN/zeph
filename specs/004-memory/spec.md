...
## Embed Backfill (v0.18.6)

> **Status**: Implemented (v0.18.6).

`SemanticMemory::embed_missing()` backfills Qdrant embeddings for messages that were stored before embeddings were available (e.g., Qdrant was offline at store time).

### Algorithm

- Processes unembedded messages in **micro-batches of 32**
- Each micro-batch uses `buffer_unordered(4)` for concurrent embedding — at most 4 embed HTTP calls in flight simultaneously
- Bounded peak memory: at most 32 messages plus their embedding vectors are live at any time
- Progress is broadcast via `tokio::sync::watch::Sender<Option<BackfillProgress>>`; `None` signals completion

### TUI Integration

When running in TUI mode, `spawn_embed_backfill()` is given a `backfill_tx` watch sender. The status bar shows `Backfilling embeddings: {done}/{total} ({pct}%)` until the backfill completes.

### Config

```toml
[llm.router]
embed_concurrency = 4   # max concurrent embed calls; 0 = unlimited (default: 4)
```

`embed_concurrency` is part of `RouterConfig` (at `[[llm.providers]]` level). Default is 4.

### Context Budget Fallback

When `context_budget_tokens` resolves to 0 (e.g., provider reports no context window), the agent falls back to **128 000 tokens** rather than disabling compaction. This prevents the compaction pipeline from never triggering on unknown providers.

### Key Invariants

- Individual embedding failures during backfill are logged and skipped — they do not abort the batch
- `None` sent on the progress channel signals completion, whether successful, timed out, or errored
- `embed_concurrency = 0` disables the semaphore (unlimited concurrency) — only safe for single-provider setups
- NEVER apply `embed_missing()` synchronously in the agent loop — always via `spawn_embed_backfill()`
...
## Kumiho: AGM Belief Revision for Graph Edges (v0.18.1)

> **Status**: Implemented (v0.18.1). Closes #2441. Migration 056.
...
## D-MEM: RPE-Based Tiered Graph Extraction Routing (v0.18.1)

> **Status**: Implemented (v0.18.1). Closes #2442.
...
## Cost-Sensitive Store Routing (v0.18.1 / updated v0.18.2)

> **Status**: Implemented (v0.18.1). Wired into `build_router()` in v0.18.2.
...
**Breaking (v0.18.2)**: `RoutingConfig` and `RoutingStrategy` are removed. Use `[memory.store_routing]` instead of the old `[memory.routing]`.
...
## CraniMem: Goal-Conditioned Write Gate (v0.18.1 / updated v0.18.2)

> **Status**: Implemented (v0.18.1). Updated v0.18.2: `goal_text` now propagated to A-MAC admission control.
...
## RL Admission Control (v0.18.1)

> **Status**: Implemented (v0.18.1). Closes #2416. Migration 055.
...
## Memex: Tool Output Archive (v0.18.1)

> **Status**: Implemented (v0.18.1). Closes #2432. Migration 054.
...
## ACON: Per-Category Compression Guidelines (v0.18.1)

> **Status**: Implemented (v0.18.1). Closes #2433. Migration 054.
...
## Spreading Activation Recall Timeout (v0.18.2)

> **Status**: Updated (v0.18.2). See also `012-graph-memory/spec.md`.
...
## Tier Promotion Locking (v0.18.2)

> **Status**: Updated (v0.18.2).
...
## Orphan Message Soft-Delete (v0.18.2)
...
## Multi-Vector Chunking (v0.18.2)

> **Status**: Implemented (v0.18.2). Closes #2551, #2552, #2570, #2571, #2586.
...
## GAAMA: Episode Nodes (v0.18.2)

> **Status**: Implemented (v0.18.2). Closes #2503, #2508.
...
## BATS: Budget Hints and Utility 5-Way Action Policy (v0.18.2)

> **Status**: Implemented (v0.18.2). Closes #2267, #2477, #2613.
...
## Focus Compression and Density-Aware Budgets (v0.18.2)

> **Status**: Implemented (v0.18.2). Closes #2553, #2510, #2481, #2604.
...
## SleepGate Forgetting Pass (v0.18.2)

> **Status**: Implemented (v0.18.2). Closes #2614.
...
## Persona Memory (v0.18.4, PR #2649 + #2652 + #2655)

> **Status**: Implemented. Source: `crates/zeph-memory/src/persona.rs`, `crates/zeph-config/src/memory.rs` (`PersonaConfig`).

Extracts user-preference and domain-knowledge facts from conversation history via a fast LLM. Facts are persisted in `persona_memory` (migration 066), deduplicated via `supersedes_id`, and injected into context assembly within `context_budget_tokens`.
...
[memory.persona]
enabled                 = false
persona_provider        = ""   # fast provider; falls back to primary
min_confidence          = 0.6
min_messages            = 3
max_messages            = 10
...
- Extraction runs at session end (`await`ed before process exit) — never dropped with `tokio::spawn`
- Self-referential heuristic gate: skip extraction when role pattern indicates A2A session
- `supersedes_id` links contradicting facts — NEVER hard-delete old facts
- Context injection is sanitized before insertion into the system prompt
- `EntityLockManager` advisory lock TTL is 120 s — NEVER hold locks across turns

---

## Trajectory Memory (v0.18.4, PR #2689 + #2692)

> **Status**: Implemented. Source: `crates/zeph-memory/src/trajectory.rs`, `crates/zeph-config/src/memory.rs` (`TrajectoryConfig`).

Extracts procedural and episodic entries from tool-call turns after each turn that contains tool use. Procedural entries are injected as "past experience" during context assembly. Episodic entries are stored per-conversation for retrospective queries.
...
[memory.trajectory]
enabled                 = false
trajectory_provider     = ""   # fast provider; falls back to primary
...
- Extraction fires only on turns containing tool calls — no-op on text-only turns
- Watermark (`last_extracted_message_id`) prevents re-extraction of already-processed turns
- NEVER block the agent loop on trajectory extraction — extraction is timeout-bounded
- `trajectory_provider` and `persona_provider` configs MUST be wired in all three agent entry points: `runner.rs`, `acp.rs`, `daemon.rs`

---

## Category-Aware Memory (v0.18.4, PR #2689)

> **Status**: Implemented. Source: `crates/zeph-memory/src/` category columns, `CategoryConfig`.

Messages are tagged with a category string derived from the active skill name or tool type and stored in `messages.category`. Qdrant recall uses `category` as a payload filter when the query is category-specific.
...
- `category` column is nullable — existing messages without a category are not excluded from recall
- Qdrant payload index on `category` must exist before filtered queries are issued

---

## TiMem — Temporal-Hierarchical Memory Tree (v0.18.4, PR #2689)

> **Status**: Implemented. Source: `crates/zeph-memory/src/tree.rs`, `TreeConfig`.

Leaf nodes are memories. Background sweeps cluster similar leaves by cosine similarity and consolidate clusters into parent summaries. Context assembly traverses the tree for complex queries.
...
- Consolidation MUST be atomic per cluster — partial writes leave dangling leaf references
- `min_cluster_size = 1` would consolidate every singleton — minimum practical value is 2
- NEVER consolidate nodes above `max_level` — tree depth is bounded

---

## Time-Based Microcompact (v0.18.4, PR #2699)

> **Status**: Implemented. Source: `crates/zeph-core/src/agent/microcompact.rs`, `MicrocompactConfig`.

Zero-LLM-cost in-memory operation. Clears low-value tool outputs (sentinel replacement) when session gap exceeds `gap_threshold_minutes`. Low-value tool set: bash, shell, grep, rg, ripgrep, glob, find, web_fetch, fetch, web_search, search, read, cat, list_directory.

### Key Invariants

- Already-cleared outputs (sentinel prefix `[cleared`) are never re-cleared
- `keep_recent` most recent compactable entries are ALWAYS preserved
- NEVER clear tool outputs that are not in the low-value set
- Runs once per turn, before LLM inference

---

## autoDream Background Consolidation (v0.18.4, PR #2697)

> **Status**: Implemented. Source: `crates/zeph-core/src/agent/autodream.rs`, `AutoDreamConfig`.

Post-session hook. Both gates must pass: `sessions_since_consolidation >= min_sessions` AND `hours_since_last >= min_hours` (passes on first run — no prior timestamp).

### Key Invariants

- Gates are in-process state only — reset on restart
- `AutoDreamState::mark_consolidated()` resets session counter to 0 and records timestamp
- Consolidation runs in a `tokio::time::timeout` bounded by `max_iterations * 30s`
- NEVER block the agent exit path — autodream fires after loop exits

---

## MagicDocs Auto-Maintained Markdown (v0.18.4, PR #2702)

> **Status**: Implemented. Source: `crates/zeph-core/src/agent/magic_docs.rs`, `MagicDocsConfig`.

Detects `# MAGIC DOC:` header in ToolOutput parts from file-read tools. Registers path per-session. Background update fires every `min_turns_between_updates` tool-call turns.

### Key Invariants

- Only one background update runs at a time — `pending.is_finished()` guard prevents overlap
- Updated file MUST contain `# MAGIC DOC:` header after write — otherwise discard
- NEVER register files read by non-file-read tools
- File-read tool set: read, file_read, cat, view, open (case-insensitive)
